...
FIGS = ROOT / "figs"
OUT  = FIGS / "fig13_o3_under_2mb.pdf"
...
# 4KB baseline = 1.0 (reference line, no bar needed)
...
ax.bar(x - w, [1.0]*n, w, color=BAR_BASE, edgecolor='none', alpha=0.7, label='4 KB baseline')
ax.bar(x,     [r['sp_o3']  for r in rows], w, color=o3_colors,  edgecolor='none', alpha=0.9, label='4 KB + O3')
ax.bar(x + w, [r['sp_2mb'] for r in rows], w, color=sp2_colors, edgecolor='none', alpha=0.85, label='2 MB baseline')
...
if 0 < n_A < n:
    ...
    ax.text((n_A - 1)/2, ax.get_ylim()[1] * 0.92,
            'Class A', ha='center', fontsize=8.5, style='italic',
            color=COLOR_A, alpha=0.8)
    ax.text(n_A + (n - n_A - 1)/2, ax.get_ylim()[1] * 0.92,
