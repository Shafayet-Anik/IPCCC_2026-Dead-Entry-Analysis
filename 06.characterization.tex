...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\figwidth]{figs/fig_reuse_distance.pdf}
  \caption{Average eviction-to-reaccess distance (log scale) per workload
  vs.\ DEPOT's default protection window, for all 21 workloads with
  measurable dead-entry activity. Hatched bars are from partially-converged
  baselines.}
  \label{fig:reuse}
\end{figure}

To connect this characterization to DEPOT's mechanism, we measure the
eviction-to-reaccess distance for each dead-entry re-walk across all 21
workloads with measurable activity (\texttt{bfs}, \texttt{gaussian},
\texttt{parboil-bfs} have none). Figure~\ref{fig:reuse} plots the average
distance per workload, log-scaled, against DEPOT's 500\,K-cycle window. All
nine TLB-sensitive workloads sit at or below the window (83--100\% of
re-walks $\geq$1K cycles after eviction), far exceeding what 1024 LRU
entries retain alone. Non-TLB-sensitive workloads span a wider range
(300 cycles to 1.66\,M), but since their miss rates are too low for stall
time to accumulate, this spread does not bear on protection efficacy.
Five hatched bars reflect not-yet-converged baselines; direction matches
the converged majority.


\noindent Reuse distance itself, however,
does not separate the two classes (\texttt{atax} and \texttt{bicg} show
comparable distributions despite their opposite DEPOT response), reinforcing
that access structure, not miss timing, is the discriminator developed next.
